...
internal class UserValidationRequestModel
{
    ...
    public Member IsValidate(string name, string password)
    {
        ...
        member = (from x in context.Members where x.Name == name && x.Password == ClaimServices.Models.EncryptDecrypt.EncodePasswordToBase64(password) select x).SingleOrDefault();
        return member;
    }
}
